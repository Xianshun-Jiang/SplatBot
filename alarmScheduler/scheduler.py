try:
    from alarmScheduler.mongoRouter  import *
    from  alarmScheduler.redisRouter import *
    import  alarmScheduler.crawler as crawler
except:
    import mongoRouter
    import redisRouter
    import crawler
...
class scheduler(object):
    def __init__(self, group = []):
        ...
        self.mode_translate_cn_to_en = {"挑战":"challenge","X":"X", "x":"X", "开放":"open"}
        self.format_rule = {"塔楼":"真格塔楼","区域":"真格区域","鱼虎":"真格鱼虎","蛤蜊":"真格蛤蜊"}
        self.group = group
        ...
        for chatroom, wxids in grouped_insts.items():
            for wxid, insturction in wxids.items():
                ...
                for text in insturction:
                    message += text + " \\n"
                request = "self.wcf.send_text(\"" + message + " \", \"" + chatroom + "\",\"" + wxid + "\")"
                retu.append(request)
        return retu
